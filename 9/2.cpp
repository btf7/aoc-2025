...
int main(int argc, char* argv[]) {
	std::cout << "Haven't solved this part yet :(" << std::endl;
	...
	// 	int dist;
	// 	if (positions[i].x == positions[i + 1].x) dist = std::abs(positions[i].y - positions[i + 1].y);
	// 	else dist = std::abs(positions[i].x - positions[i + 1].x);
	// 	if (dist < 10) std::cout << dist << " ";
	// }
